...
class CFG:
    ...
    # LLRD for partial unfreeze
    unfreeze_last_n = 4  # number of last blocks to unfreeze
    llrd_decay = 0.8     # layer-wise lr decay
    warmup_ratio = 0.078
...
def train_fold(fold, train_df, cfg, device="cuda"):
    ...
    unfreeze_from = max(0, n_blocks - cfg.unfreeze_last_n)
    ...
    if hasattr(model.backbone, "norm"):
        for p in model.backbone.norm.parameters():
            p.requires_grad = True
        norm_params = list(model.backbone.norm.parameters())
    
    # Head params (always train)
    ...
    # Build LLRD param groups for unfreezed blocks
    param_groups = []
    for i in range(unfreeze_from, n_blocks):
        lr_i = cfg.lr * cfg.backbone_lr_mult * (cfg.llrd_decay ** (n_blocks - 1 - i))
        params_i = [p for p in model.backbone.blocks[i].parameters() if p.requires_grad]
        if params_i:
            param_groups.append({'params': params_i, 'lr': lr_i})
    
    if norm_params:
        param_groups.append({'params': norm_params, 'lr': cfg.lr * cfg.backbone_lr_mult})
    
    # Head params with base lr
    param_groups.append({'params': head_params, 'lr': cfg.lr})
    
    optimizer = AdamW(param_groups, weight_decay=cfg.weight_decay)

    print(f"  LLRD: unfreeze_last_n={cfg.unfreeze_last_n}, decay={cfg.llrd_decay}")
    print(f"  Backbone blocks: {n_blocks}, unfreeze from: {unfreeze_from}")
